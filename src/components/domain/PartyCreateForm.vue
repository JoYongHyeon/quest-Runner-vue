...
import {partyApi, type PartyCreateReqDTO, type PartySlotCreateDTO} from '../../api/partyApi';
...
const loadPartyDetail = async (id: number) => {
  try {
    ...
    // [주의] 백엔드에서 techStacks를 주지 않아 빈 배열로 초기화됨
...
const handleSubmit = async () => {
  try {
    ...
    if (props.partyId) {
      ...
      // [생성 모드]
      const res = await partyApi.createParty(form.value);
      alert('새로운 퀘스트가 생성되었습니다! 🎉');
      router.push(`/party/${res.partyId}`); // 생성된 상세 페이지로 이동
    }
  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="p-5 bg-gray-50 rounded-xl border border-gray-200 dark:bg-[#202022] dark:border-[#343536]">
      ...
      <div class="flex flex-col gap-4 mb-6">
        ...
        <div class="w-full">
          ...
          <div v-if="currentSlot.techStacks.length > 0"
               class="mt-3 flex flex-wrap gap-1.5 p-3 bg-white border rounded-lg dark:bg-[#272729] dark:border-[#343536]">
            ...
            <span v-for="(tech, idx) in currentSlot.techStacks" :key="idx"
                  class="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs font-bold flex items-center gap-1 border border-blue-100 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800">
                          ...
                          <button type="button" @click="toggleTech(tech)"
                                  class="hover:text-red-500 font-normal">×</button>
                      </span>
          </div>
      ...
      <!-- 추가된 슬롯 리스트 -->
      <div class="space-y-3">
        <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wide">생성된 슬롯 목록 ({{ form.slots.length }} /
          {{ MAX_SLOTS }})</h4>
        ...
        <div v-for="(slot, index) in form.slots" :key="index"
          ...
          <div class="flex flex-col gap-2">
            <div class="flex items-center gap-2">
                          <span
                              class="px-2 py-0.5 rounded bg-gray-100 text-gray-700 text-xs font-bold border border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
                              {{ slot.position }}
                          </span>
        ...
        <div v-if="form.slots.length === 0"
             class="py-8 border-2 border-dashed border-gray-200 rounded-lg text-center dark:border-[#343536]">
    ...
    <div class="p-5 bg-blue-50 rounded-xl border border-blue-100 dark:bg-blue-900/10 dark:border-blue-800">
      ...
      <div class="space-y-2">
        ...
        <div v-if="form.linkList.length === 0"
             class="text-center py-4 text-sm text-blue-400 opacity-60 border border-dashed border-blue-200 rounded-lg dark:border-blue-800/50">
